...
These definitions are used throughout the remainder of this manual.`POSIX`
...
A `token` that performs a control function. It is a `newline` or one of the following: ‘\|\|’, ‘&&’, ‘&’, ‘;’, ‘;;’, ‘;&’, ‘;;&’, ‘\|’, ‘\|&’, ‘\(’, or ‘\)’.`exit status`
...
A character that, when unquoted, separates words. A metacharacter is a `space`, `tab`, `newline`, or one of the following characters: ‘\|’, ‘&’, ‘;’, ‘\(’, ‘\)’, ‘&lt;’, or ‘&gt;’.`name`

A `word` consisting solely of letters, numbers, and underscores, and beginning with a letter or underscore. `Name`s are used as shell variable and function names. Also referred to as an `identifier`.`operator`

A `control operator` or a `redirection operator`. See [Redirections](redirections-bash-reference-manual.md#Redirections), for a list of redirection operators. Operators contain at least one unquoted `metacharacter`.`process group`
...
A `word` that has a special meaning to the shell. Most reserved words introduce shell flow control constructs, such as `for` and `while`.`return status`
...
A shell builtin command that has been classified as special by the POSIX standard.`token`
